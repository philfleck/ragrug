...
namespace RR
{
    ...
    public class Runtime : BaseManager<Runtime>
    {
        ...
        // TODO: Migrate to away from Trackers and explicit Datasets
        // For reference: https://library.vuforia.com/project-migration/how-migrate-unity-project
            ...
            JSCallback targetLostCb, string loadThisDataset)
        {
            ...
            // Accessing dynamically created trackables and re-assinging them
        ...
        public IEnumerator LateInitVufo(string targetGo, string loadThisDataset)
        {
            ...
            var vb = Camera.main.GetComponent<VuforiaBehaviour>();
            vb.enabled = true;
        ...
        public static Microsoft.MixedReality.Toolkit.UI.ObjectManipulator ToggleObjManipulationWithFeedback(string goName, JSCallback onManipulationStarted, JSCallback onManipulationEnded)
        {
            var go = GameObject.Find(goName);
            if (go != null)
            {
                ...
                {
                    c.OnManipulationStarted.AddListener((x) => {
                        onManipulationStarted?.Invoke(
                           new Jint.Native.JsValue("onManipulationStarted"),
                           new Jint.Native.JsValue[] { goName, x.ManipulationSource.name, x.IsNearInteraction });
                    });
                }

                if (onManipulationEnded != null)
                {
                    c.OnManipulationEnded.AddListener((x) => {
                        onManipulationEnded?.Invoke(
                           new Jint.Native.JsValue("onManipulationEnded"),
                           new Jint.Native.JsValue[] { goName, x.ManipulationSource.name, x.IsNearInteraction });
                    });
                }
        ...
        public static Microsoft.MixedReality.Toolkit.Input.NearInteractionGrabbable ToggleNearGrab(string goName)
        {
            var go = GameObject.Find(goName);
            if (go != null)
            {
                var c = ToogleComponent<Microsoft.MixedReality.Toolkit.Input.NearInteractionGrabbable>(go);
                return c;
            }
            return null;
        }
        ...
        public static Rigidbody AddRigidbody(GameObject go)
        {
            if (go != null)
            {
        ...
        public static void SetAnimationController(UnityEngine.Animator animator, RuntimeAnimatorController rac)
        {
            try
            {
                if (animator != null)
            ...
            {
                Debug.Log("SetAnimationController ERROR => " + e);
            }
        }
