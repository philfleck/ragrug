...
namespace Vizario
{
    ...
    public class MQTTManager : BaseManager<MQTTManager>
    {
        ...
        public void Update()
        {
            ...
            {
                var N = Math.Min(updateCBs.Count, 10);
                for (var i = 0; i < N; i++)
                {
        ...
        public int RegisterCallbackTopicImple(Callbackholder fct, string topic)
        ...
        public void UnregisterCallbackTopicImple(int index, string topic)
        {
            if (topicCallbacks.TryGetValue(topic, out Dictionary<int, Callbackholder> cbs))
            {
                cbs.Remove(index);
            }
        }
        ...
        public static int RegisterCallbackTopic(JSCallback fct, string topic)
        {
            return GetRTInstance().RegisterCallbackTopicImple(new Callbackholder(fct), topic);
        }

        public static int RegisterCallbackTopicCs(CSCallback fct, string topic)
        {
            return GetRTInstance().RegisterCallbackTopicImple(new Callbackholder(fct), topic);
        }
        ...
        public static void UnregisterCallbackTopic(int index, string topic)
        {
            GetRTInstance().UnregisterCallbackTopicImple(index, topic);
		}
