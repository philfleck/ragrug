...
namespace IATK
{
    public abstract class AbstractVisualisation : MonoBehaviour
    {
        ...
        protected void BindMinMaxAxisValues(Axis axis, DimensionFilter dim)
        {
            ...
            //} catch(Exception err)
            //{
            //    Debug.LogError("AbstractVis ERROR BindMinMaxAxisValues no values found to bind => " + dim.Attribute + " => " + err);
            //}
        }
            ...
            switch (configuration.Geometry)
            {
                ...
                case AbstractVisualisation.GeometryType.Lines:
                    if (visualisationReference.graphDimension != "Undefined" )
                    {
                        ...
                        /*
                        if(visualisationReference.dataDimensions.GetType().Equals(typeof(CSVDataSource)))
                        {
                            CSVDataSource csvds = (CSVDataSource)(visualisationReference.dataSource);
                            builder.createIndicesGraphTopology(csvds.GraphEdges);
                            mt = new Material(Shader.Find("IATK/LinesShader"));
                        ...
                        if (visualisationReference.dataDimensions.GetType().Equals(typeof(RealtimeDataSource)))
                        {
                            RealtimeDataSource rtds = (RealtimeDataSource)(visualisationReference.dataSource);
                            builder.createIndicesGraphTopology(rtds.GraphEdges);
                            mt = new Material(Shader.Find("IATK/LinesShader"));
                            mt.renderQueue = 3000;
                            mt.enableInstancing = true;
                        }
                        */
                        return builder.updateView().apply(gameObject, mt);
                    }
                    else
