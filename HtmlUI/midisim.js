...
function MIDI_INIT() {
    ...
    //RT.Unity.SetLocalPose(MI.go, [0, 0, 0], [0,0,0,1], null);
    ...
    */
    ...
    RT.Web.SetTiimeout(1000, function () {
        console.log("midiclient before CNR");
        CNR_InitFromId("89ca6e54.b8f36");
    });
}
...
function MIDI_InitSoundChart() {
    ...
    RT.Web.SetTiimeout(50, function () {
        try {
            ...
            RT.Unity.SetLocalPose(MI.chartData.chartGo, [-1250, 460, 0], [-0.25, 0, 0, 0.96], null);
        } catch (err) {
...
function MIDI_Init3DButtons() {
    try {
        ...
        RT.Unity.SetLocalPose(plusBtn.go, [-0.02, 0.07, -0.01], [-0.25, 0, 0, 0.96], [1, 1, 1]);
        RT.Unity.SetLocalPose(minusBtn.go, [0.02, 0.07, -0.01], [-0.25, 0, 0, 0.96], [1, 1, 1]);

        RT.Unity.SetLocalPose(freqUpBtn.go, [0.06, 0.07, -0.01], [-0.25, 0, 0, 0.96], [1, 1, 1]);
        RT.Unity.SetLocalPose(freqDownBtn.go, [0.1, 0.07, -0.01], [-0.25, 0, 0, 0.96], [1, 1, 1]);

        //RT.Unity.SetLocalPose(tmBtn.go, [0.16, 0, -0.01], [0, 0, 0, 1], [1, 1, 1]);
        //RT.Unity.SetLocalPose(lnBtn.go, [0.2, 0.1, -0.01], [0, 0, 0, 1], [1, 1, 1]);
